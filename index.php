...
function launch() {
    ...
    shell_exec('sudo raspivid --awb "'.$WB.'" -ev "'.$EV.'" -ss  "'.$SS.'"  --ISO  "'.$iso.'"  -t 0 -h  "'.$H.'" -w  "'.$W.'" -fps  "'.$FPS.'"  -b 4500000 -o - | gst-launch-1.0 -v fdsrc ! h264parse ! rtph264pay config-interval=1 pt=96 ! gdppay ! tcpserversink host= "'.$ip.'" port=5000 > /dev/null &');
}
